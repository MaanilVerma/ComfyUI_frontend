...
describe('isLinkedPromotion', () => {
  ...
  it('matches linked widget from nested subgraph with disambiguatingSourceNodeId', () => {
    const subgraphNode = createSubgraphWithInputs()
  ...
  it('returns false for independent promotion not bound to any input', () => {
    const subgraphNode = createSubgraphWithInputs()
    subgraphNode.inputs[0]._widget = linkedWidget('3', 'string_a')

    expect(isLinkedPromotion(subgraphNode, '3', 'string_a')).toBe(true)
    expect(isLinkedPromotion(subgraphNode, '4', 'value')).toBe(false)
  })
})
